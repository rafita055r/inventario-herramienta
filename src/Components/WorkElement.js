...
export default function WorkElement({
  ...
  return (
    <li className="li_Work">
      <h3
        className={isOpen ? "listToolOpen" : null}
      ...
      <div className={`tools-container ${isOpen ? "open" : ""}`}>
        <div className="contain_btnDel_btnUpd">
          <button
            className="btn_editWork"
            onClick={() => setWorkDataToEdit({ ...dataWork })}
          >
            Editar
          </button>
          <button
            className="btn_delWork"
            onClick={() => setWorkDataToDel({ ...dataWork })}
          >
            Eliminar
          </button>
        </div>
        {
          <ul className="listTools">
            {herramientas_enObra.length > 0 ? (
              herramientas_enObra.map((tool) => {
