...
export default function ToolsRoute({ toolsList }) {
  ...
  useEffect(() => {
    if (dataSearching.trim() === "") {
      setNotFoundTool(false);
      setResultSearch([]);
    }
  }, [dataSearching]);
  return (
    <div className="main-tool">
      {showForm && <FormAddTool showForm={handleShowForm} />}

      <button className="add-tool-btn" onClick={() => setShowForm(!showForm)}>
        {showForm ? "Cancelar" : "Agregar herramienta"}
      </button>

      <div className="contain-search-input">
        <input
          type="text"
          name="search-tool"
          className="input-search-tool"
          placeholder="Buscar herramientas"
          defaultValue={dataSearching}
          onChange={handleSearching}
        />
        <button onClick={onClickSearch} className="btn-search-tool">
          <IoMdSearch />
        </button>
      </div>
      ...
      <section className="sectionTools">
        ...
        <ul className="ul_tools">
          {loading ? (
            <LoaderMain />
          ) : resultSearch.length && dataSearching.trim() !== "" ? (
            resultSearch.map((tool) => (
          ...
          ) : !resultSearch.length && notFoundTool ? (
            <span>
              No se encontraron herramientas con el nombre "{dataSearching}"
            </span>
          ) : toolsList.length > 0 ? (
            toolsList.map((tool) => (
              <li key={tool.id} onClick={() => toolDetailRedirectPage(tool)}>
